...
TelnetLogin
    FOR    ${portlistfromansible}    IN    @{portlistfromansible}
        ...
        Run Keyword If    ${status}==True    Set Tags    ${portlistfromansible} login is successful and up 
        Run Keyword If    ${status}==False   Set Tags   ${portlistfromansible} login is Unsuccesful and down
        Close Connection   
    END
